...
def find_possible_steps(current_xy, prev_steps):
    ...
    possibles = sorted(sorted(possibles, key=lambda x: x[2], reverse=True), key=lambda x: x[3], reverse=True)

    for possible in possibles:
        if max_x >= possible[0] >= 0 and max_y >= possible[1] >= 0 and ((current_h <= data[possible[1]][possible[0]] <= chr(ord(current_h) + 1) and data[possible[1]][possible[0]] != 'E') or (current_h in ['y', 'z'] and data[possible[1]][possible[0]] == 'E')):
...
def do_step(start_xy, prev_steps, level):
    ...
    print(level, start_xy, data[start_xy[1]][start_xy[0]])
    ...
    for next_step in next_steps:
        ...
        if data[next_step[1]][next_step[0]] == 'E':
